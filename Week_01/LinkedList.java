 ...
 * methods of the List interface, this class provides access to the first * and
 * last list elements in O(1) time for easy stack, queue, or double-ended *
 ...
 * non-deterministic behavior. * * @author Original author unknown * @author
 * Bryce McKinlay * @author Eric Blake (ebb9@email.byu.edu) * @author Tom Tromey
 * (tromey@redhat.com) * @author Andrew John Hughes (gnu_andrew@member.fsf.org)
 * * @see List * @see ArrayList * @see Vector * @see
 * Collections#synchronizedList(List) * @since 1.2 * @status Complete to 1.6
 */
...
public class LinkedList<T> extends AbstractSequentialList<T> implements List<T>, Deque<T>, Cloneable, Serializable {
    ...
    /**
     * Obtain the Entry at a given position in a list. This method of course takes
     * linear time, but it is intelligent enough to take the shorter of the paths to
     * get to the Entry required. This implies that the first or last entry in the
     * list is obtained in constant time, which is a very desirable property. For
     * speed and flexibility, range checking is not done in this method: Incorrect
     * values will be returned if (n &lt; 0) or (n &gt;= size).
     *
    ...
    /**
     * Append the elements of the collection in iteration order to the end of this
     * list. If this list is modified externally (for example, if this list is the
    ...
    /**
     * Insert the elements of the collection in iteration order at the given index
     * of this list. If this list is modified externally (for example, if this list
     ...
     * this LinkedList. If the passed-in Array is not large enough to store all of
     ...
     * @throws ArrayStoreException  if the runtime type of a does not allow an
     ...
     * @serialData the size of the list (int), followed by all the elements (Object)
     *             in proper order
     */
    ...
    /**
     * A ListIterator over the list. This class keeps track of its position in the
     * list and the two list entries it is between.
     *
    ...
    public Iterator<T> descendingIterator() {
        return new Iterator<T>() {
            ...
            /**
             * Checks for modifications made to the list from elsewhere while iteration is
             * in progress.
             *
             ...
             * @throws NoSuchElementException          if there are no more objects to
             *                                         retrieve.
             * @throws ConcurrentModificationException if the list has been modified
             *                                         elsewhere.
             */
            ...
            /**
             * Removes the last object retrieved by <code>next()</code> from the list, if
             * the list supports object removal.
             *
             * @throws ConcurrentModificationException if the list has been modified
             *                                         elsewhere.
             * @throws IllegalStateException           if the iterator is positioned before
             *                                         the start of the list or the last
    ...
    /**
     * Pops an element from the stack by removing and returning the first element in
     * the list. This is equivalent to calling {@link #removeFirst()}.
     *
    ...
    /**
     * Pushes an element on to the stack by adding it to the front of the list. This
     * is equivalent to calling {@link #addFirst(T)}.
     *
     ...
     * traversing the list from head to tail. The list is unchanged if the element
     * is not found. This is equivalent to calling {@link #remove(Object)}.
     *
